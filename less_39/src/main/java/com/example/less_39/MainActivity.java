...
public class MainActivity extends Activity {
    ...
    private void writeStaff(SQLiteDatabase db) {
        ...
        String sqlQuery = "select PL.name as Name, PS.name as Position, " +
                "salary as Salary " +
                "from people as PL " +
            ...
            if (oldVersion == 1 && newVersion == 2) {
                ...
                try {
                    ...
                    for (int i = 0; i < position_id.length; i++) {
                        ...
                        db.update("people", contentValues, "position = ?",
                                new String[] { position_name[i] });
                    }

                    db.execSQL("create temporary table people_temp (" +
                            "id integer, name text, position text, posid integer);");

                    db.execSQL("insert into people_temp select id, name, position, posid from people;");
                    ...
                    db.execSQL("insert into people select id, name, posid from people_temp;");
                    db.execSQL("drop table people_temp;");
